...
# Set conf file if env var are set
if [[ ! -z "$CONF" && ! -f "config.yml" ]];then
    echo "Set conf"
    echo -e "$CONF" > config.yml
else
    echo '$CONF not set or file exist'
fi
...
if [[ "$1" = "api" || -z $1 ]];then
    ...
    exec celery worker -A celery_launch.cel -P solo --maxtasksperchild=$CELERY_WORKER_MAX_TASKS
    ...
    exec celery beat -A celery_launch.cel
